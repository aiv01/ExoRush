    ...
    void Start()
    {
        ...
        for(int i = 0; i < SpawingObject.Length; i++)
        {
            ...
            if(((transform.position.z / (100000 * DistanceReducer))*SpawnRateMuiltiplier) > (Random.Range(0, Random.value)))
                ...
                {
                    Instantiate(EnemyLibrary[Random.Range(0, EnemyLibrary.Length)], SpawingObject[i].transform.position, Quaternion.Euler(0, 220, 0));//SpawingObject[i].transform.rotation);
                }
